...
# we may also want the reverse complement of the target
...
for $K (0..$ENDRANGE) {
    ...
    # by default bioperl doesn't give this, though it is in the output file, so the output file must be used.
